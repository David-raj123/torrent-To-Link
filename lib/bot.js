...
const startMessage = `
Hi , I Am A Torrent To Link Generater Bot . 
Use Below Cammands To Use Me .


===============================

1. Convert Torrent Magnetic Link To Direct Download Link :-
...
/status {magnet link} - To Check Status Of A Downloading Torrent Info Hash Is Provided When Torrent Download Starts.
...
function bot(torrent, bot) {
  ...
  bot.onText(downloadRegex, (msg, match) => {
    ...
    const reply = async torr => {
      ...
      if (!torr.done) {
        ...
        mess1 += `⭕ Time Status :- ${torr.redableTimeRemaining}\n\n`;
      } else {
  ...
  bot.onText(statusRegex, (msg, match) => {
    ...
    if (link.indexOf("magnet:") !== 0) {
      ...
      bot.sendMessage(from, "Not downloading yet!");
    } else {
      let mess1 = "";
  ...
  bot.onText(removeRegex, (msg, match) => {
    ...
    try {
      torrent.remove(link);
      bot.sendMessage(from, "Removed from Query");
    } catch (e) {
